...
class TwimlController < ApplicationController
  ...
  def ask_region
    # TwiMLを作成
    xml_str = Twilio::TwiML::Response.new do |response|
      response.Gather timeout: 40, finishOnKey: '', numDigits: 1, action: "#{Settings.app_host}/twiml/question?q_num=1&user_id=#{current_user.id}" do |gather|
  ...
  def question
    ...
    if q_num == 1
      ...
      current_user.update_attributes({
        "cond_#{q_num - 1}".to_sym => last_answered_conditon_id, # :cond_2 => 1
      })
    end

    current_4conditions = current_user.get_next_conditions!

    api_request = ApiRequest.generate(current_user.generate_condition_for_request)
